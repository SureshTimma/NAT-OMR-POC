...
def draw_phone_column(c, x, y, width, height, title, rows=10, value=None):
    ...
    for i in range(rows):
        ...
        # Fill digit if value provided
        # DISABLED TEXT FILLING
        # if value and i < len(value):
        #     digit_char = value[i]
        #     c.setFont("Helvetica-Bold", 10)
        #     c.setFillColor(colors.black)
        #     c.drawCentredString(bx + box_w/2, write_y + 4, digit_char)
    ...
    for i in range(rows): 
        ...
        for digit in range(10): 
            ...
            if digit == target_digit:
                 # Fill Bubble
                 c.setFillColor(colors.black)
                 c.circle(col_center_x, by, bubble_radius, stroke=1, fill=1)
                 # No Text inside filled bubble
            else:
                 # Empty Bubble
                 c.setFillColor(TEXT_COLOR)
                 c.circle(col_center_x, by, bubble_radius, stroke=1, fill=0)
                 # RESTORED TEXT IN EMPTY BUBBLE
                 c.setFillColor(TEXT_COLOR)
                 c.drawCentredString(col_center_x, by - 2.5, str(digit))


def draw_continuous_questions(c, x, y, width, height, answers=None):
    ...
    for col in range(num_cols):
        ...
        for r in range(q_per_col):
            ...
            for i, opt in enumerate(options):
                ...
                # Enhanced Fill Types
                val = answers.get(str(q_num)) if answers else None
                fill_type = "EMPTY"
                if val:
                    if isinstance(val, list):
                        if opt in val: fill_type = "FULL"
                    elif isinstance(val, tuple):
                        # Expecting (Opt, "HALF")
                        if val[0] == opt and len(val) > 1 and val[1] == "HALF": fill_type = "HALF"
                    elif val == opt:
                        fill_type = "FULL"
                ...
                if fill_type == "FULL":
                     ...
                     c.setFillColor(colors.black)
                     # Wedge (Semicircle Top)
                     c.wedge(cx - bubble_radius, cy - bubble_radius, cx + bubble_radius, cy + bubble_radius, 0, 180, stroke=0, fill=1)
                     # Outline
                     c.circle(cx, cy, bubble_radius, stroke=1, fill=0)
                     c.setFillColor(TEXT_COLOR)
                     c.drawCentredString(cx, cy - 2, opt)
                else: # EMPTY
                     c.setFillColor(TEXT_COLOR)
                     c.circle(cx, cy, bubble_radius, stroke=1, fill=0)
                     c.setFillColor(TEXT_COLOR)
...
def generate_filled_continuous_60q_omr():
    filename = "omr_sheet_continuous_60q_filled.pdf"
    ...
    # Values to bubble
    import random
    random.seed(None) # Reset default system source
    phone1 = "".join([str(random.randint(0, 9)) for _ in range(10)])
    phone2 = "".join([str(random.randint(0, 9)) for _ in range(10)])
    print(f"Generated Random Phone 1: {phone1}")
    print(f"Generated Random Phone 2: {phone2}")
    
    draw_phone_column(c, margin, body_y_top - phone_height, phone_width, phone_height, "Phone Number (WhatsApp No.)", value=phone1)
    draw_phone_column(c, margin + phone_width + phone_gap, body_y_top - phone_height, phone_width, phone_height, "Parent / Guardian Phone No.", value=phone2)
    ...
    for i in range(1, 61):
        q = str(i)
        if 1 <= i <= 10:
            answers[q] = random.choice(options_list) # Full
        elif 11 <= i <= 15:
            answers[q] = None # Unmarked
        elif 16 <= i <= 20:
            # Double Options
            o = random.sample(options_list, 2)
            answers[q] = o
        elif 21 <= i <= 25:
            # Half Filled
            o = random.choice(options_list)
            answers[q] = (o, "HALF")
        else:
            answers[q] = random.choice(options_list) # Full

    print("Generated Mixed Answers:")
    print(answers)
    
    # Save Generated Truth (serialize list/tuple)
    def serialize(v):
        if isinstance(v, tuple): return list(v)
        return v
    with open("generated_answers.json", "w") as f:
        import json
        json.dump({k: serialize(v) for k,v in answers.items()}, f, indent=4)
        
    draw_continuous_questions(c, margin, questions_y_top - total_grid_h, content_w, total_grid_h, answers=answers)
    ...
    # Different footer to confirm rebuild
    c.drawRightString(w - margin, 5, "OMR_UPDATED_CONTINUOUS_60Q_V10_V15_MIXED_CASES")
...
if __name__ == "__main__":
    generate_filled_continuous_60q_omr()

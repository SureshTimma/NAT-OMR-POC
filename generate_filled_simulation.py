...
def draw_phone_column(c, x, y, width, height, title, rows=10):
    ...
    for i in range(rows): # Columns 0-9 (digits of phone number)
        ...
        for digit in range(10): # Rows 0-9
            ...
            # SIMULATION: Fill bubbles for phone number 9876543210
            # Col 0: 9, Col 1: 8, ...
            sim_phone = "9876543210"
            fill_val = 0
            if i < len(sim_phone) and str(digit) == sim_phone[i]:
                fill_val = 1
            
            c.circle(col_center_x, by, bubble_radius, stroke=1, fill=fill_val)
    ...
    for col in range(num_cols):
        ...
        for r in range(num_q):
            ...
            for i, opt in enumerate(options):
                ...
                # SIMULATION: Fill 'A' (index 0) for all questions
                fill_val = 0
                if i == 0:
                    fill_val = 1
                    
                c.circle(bx, qy, bubble_radius, stroke=1, fill=fill_val)
...
def generate_advanced_omr():
    filename = "tests/omr_filled_simulation.pdf"
